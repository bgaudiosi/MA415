...
tmpFn <- function(xVec) {
  len <- length(xVec)
  series <- (xVec[1:(len-2)] + xVec[2:(len-1)] + xVec[3:len])/3
  return(series)
}
```
...
```{r}
tmpFn <- function(xVec) {
  return(ifelse(xVec < 0, xVec^2  + 2*xVec + 3, ifelse(xVec >= 2, xVec^2 + 4*xVec -7, xVec + 3)))
}
xy <- seq(-3,3,len=50)
plot(xy,tmpFn(xy),type="l")
```
...
```{r}
doubleMatrix <- function(matA) {
  return(ifelse(matA %% 2 == 1, 2*matA, matA ))
}
matA <- matrix(c(1,1,3,5,2,6,-2,-1,-3), nrow=3, byrow=TRUE)
matA
doubleMatrix(matA)
...
zeller <- function(day, month, year) {
  ...
  y <- year %% 100
  c <- year %/% 100
  m <- month - 2 + 12*(month < 3)
  result <- (floor(2.6*m - 0.2) + k + y  + floor(y/4) + floor(c/4) - 2*c) %% 7
  return(result)
}
zeller(1,2,2018)
zeller(2,2,2018)
